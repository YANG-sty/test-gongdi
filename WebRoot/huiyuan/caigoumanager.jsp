	...
	<div class="search-options">
		<form id="searchForm"
			...
			<table cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td>材料编号 <input name="spno" value="${spno}" class="input-txt"
							type="text" id="spno" /> <input type="hidden" name="actiontype"
							value="search" />
                           <input type="hidden" name="hyid"
							value="${huiyuan.id}" /> <input type="hidden" name="forwardurl"
							value="/huiyuan/caigoumanager.jsp" />
							<div class="ui-button">
		...
		<tbody>
			...
			<tr>
				<td>&nbsp<input id="chk<%=temcaigou.getId()%>" class="check"
					...
					value='<%=temcaigou.getId()%>'>
				</td>
				<td><%=temcaigou.getSpno()%></td>
				<td><%=temcaigou.getSpname()%></td>
				<td><%=temcaigou.getSupplor()%></td>
				<td><%=temcaigou.getCount()%><%=temcaigou.getDanwei() %></td>
				<td><%=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(temcaigou.getCgdate())%></td>
				<td><%=temcaigou.getCgren()%></td>
				<td> <a class="btn btn-success"
					href="${pageContext.request.contextPath}/admin/caigoudetails.jsp?id=<%=temcaigou.getId()%>"><i
